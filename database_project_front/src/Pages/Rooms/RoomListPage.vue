...
const applyFilters = () => {
    router.replace({ path: '/rooms', query: buildParams() });
};
        ...
        <section class="space-y-6">
            <div class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                    ...
                    <Link to="/dashboard" class="rounded-2xl border border-slate-900 bg-slate-900 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-700">
                        返回儀表板
                    </Link>
                </div>
                ...
                <div v-if="rooms.data.length" class="divide-y divide-slate-200">
                    <div v-for="room in rooms.data" :key="room.id" class="grid grid-cols-1 gap-4 px-6 py-5 lg:grid-cols-[1.5fr_1fr_1fr_180px] lg:items-center">
                        ...
                        <div class="flex justify-start gap-2 lg:justify-end">
                            <Link :to="`/rooms/${room.id}`" class="rounded-xl border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-100">
                                查看詳情
                            </Link>
                        </div>
                    </div>
